(function ($) {
    ...
    var halo = {
        ...
        init: function () {
            ...
            // Cart Page Coupon Validator
            if($('.cart-total-coupon').length){
              this.cartCouponValidator();
            }
            // Init Mobile Menu 
        ...
        initColorSwatch: function() {
            ...
            $doc.on('click', '.card .swatch-label', (event) => {
                var $target = $(event.currentTarget),
                    ...
                    // $(event.target.closest('.product-item')).find('.card-media .ls-is-cached').not(':first-child').filter(':not(:only-child)').css('display', 'none');     
                    // $(event.target.closest('.product-item')).find(".card-media").addClass("cardColorPicked");
                ...
                if (productJson.media != undefined) {
                   console.log("title-media-start");
                   console.log(productJson.media);
                    // var mediaList = productJson.media.filter((index, element) => {
                    //     console.log(element.index);
                    //     return element.alt === title;
                    // });
                    var mediaList = productJson.media.filter((element, index) => {
                        console.log(index);
                        return element.alt === title;
                    });
                }
        ...
        initQuickView: function(){
            ...
            $doc.on('click', '[data-open-quick-view-popup]', (event) => {
                ...
                halo.updateContentQuickView(handle,product_id);
            });
        ...
        updateContentQuickView: function(handle,product_id=null){
            ...
            $.ajax({
                ...
                complete: function () {
                    ...
                    console.log("quick view complete upto variant");
                    ...
                    items.each((index, element) => {
                        ...
                        // var active_title = $("#product-swatch-"+this.dataset.product).find('.swatch-label.is-active').attr("title");
                  var active_title = $("#product-swatch-"+product_id).find('.swatch-label.is-active').attr("title");
                  if(active_title){
                    $scope.find('input[name="Color"]').prop('checked', false);
                    $scope.find('input[name="Color"][value="'+active_title+'"]').prop('checked', true);
                  }
        ...
        cartCouponValidator: function(){
            console.log("cart coupon validator");
          let clearBtn = document.querySelector("#clear-discount-btn");
          let applyBtn = document.querySelector("#apply-discount-btn");
          let discountCodeError = document.querySelector("#discount-code-error");
          let discountCodeWrapper = document.querySelector("#applied-discount-code .applied-discount-code-wrapper");
          let discountCodeValue = document.querySelector(".discount-label");
          ...
          if (localStorage.discountCode) fetchDiscount( JSON.parse(localStorage.discountCode).code,false);
          if(applyBtn) applyBtn.addEventListener("click", function(e){
              e.preventDefault();
              console.log("applied");
              console.log(discountCodeInput.value);
              if(discountCodeInput.value.length=="") discountCodeError.innerHTML = "Enter the discount code"
              if(discountCodeInput.value.length>1) applyDiscount(discountCodeInput.value,true)
            });
          function applyDiscount(code, state){
            if(applyBtn) {
                    applyBtn.innerHTML = "APPLYING",applyBtn.style.pointerEvents = "none";
                  }
                  fetch("/payments/config", {"method": "GET"}).then(function(response) { return response.json() })
                    .then(function(data) {
                    this.checkout_json_url = '/wallets/checkouts/';
                    this.authorization_token = btoa(data.paymentInstruments.accessToken)
                    Shopify.getCart((cart) => {
                    this.check_payload = {"checkout": { "country": Shopify.country,"discount_code": code,"line_items": cart.items, 'presentment_currency': Shopify.currency.active } }
                      fetch(this.checkout_json_url, {"headers": {"accept": "*/*", "cache-control": "no-cache","authorization": "Basic " + this.authorization_token,"content-type": "application/json, text/javascript","pragma": "no-cache", "sec-fetch-dest": "empty","sec-fetch-mode": "cors", "sec-fetch-site": "same-origin"},"referrerPolicy": "strict-origin-when-cross-origin","method": "POST", "mode": "cors", "credentials": "include","body": JSON.stringify(this.check_payload) }).then(function(response) { return response.json() })
                     .then(function(data) {
                        if(data.checkout && data.checkout.applied_discounts.length > 0){
                          let discountApplyUrl = "/discount/"+code+"?v="+Date.now()+"&redirect=/checkout/";
                          fetch(discountApplyUrl, {}).then(function(response) { return response.text(); })
                          document.querySelector(".custom-discount").style.display = "flex";
                          if(data.checkout.applied_discounts[0].title){
                          if(discountCodeValue) discountCodeValue.innerHTML = data.checkout.applied_discounts[0].title + " "+'<button id="clear-discount-btn" class="close-icon ">×</button>';                          
                          let localStorageValue = {'code': data.checkout.applied_discounts[0].title,'totalCart': data.checkout.total_line_items_price};
        ...
        if(applyBtn){
          ...
          function fetchDiscount(code, state){
                      if(applyBtn) {
                              applyBtn.innerHTML = "APPLYING",applyBtn.style.pointerEvents = "none";
                            }
                            fetch("/payments/config", {"method": "GET"}).then(function(response) { return response.json() })
                              ...
                              this.authorization_token = btoa(data.paymentInstruments.accessToken)
                              Shopify.getCart((cart) => {
                              this.check_payload = {"checkout": { "country": Shopify.country,"line_items": cart.items, 'presentment_currency': Shopify.currency.active } }
                                fetch(this.checkout_json_url, {"headers": {"accept": "*/*", "cache-control": "no-cache","authorization": "Basic " + this.authorization_token,"content-type": "application/json, text/javascript","pragma": "no-cache", "sec-fetch-dest": "empty","sec-fetch-mode": "cors", "sec-fetch-site": "same-origin"},"referrerPolicy": "strict-origin-when-cross-origin","method": "POST", "mode": "cors", "credentials": "include","body": JSON.stringify(this.check_payload) }).then(function(response) { return response.json() })
                               .then(function(data) {
                                   console.log(data);
                                  if(data.checkout && data.checkout.applied_discounts.length > 0){
          ...
          function clearDiscount() {
            ...
            let discountApplyUrl = "/discount/null?v="+Date.now()+"&redirect=/checkout/";
            fetch(discountApplyUrl, {}).then(function(response) { return response.text(); })
            //fetch("/checkout?discount=%20null")
              .finally(function(params) {
               console.log("finaly");
              Shopify.getCart((cart) => {halo.updateCart(cart);});
                  document.querySelector(".custom-discount").style.display = "none";
            });
          }
          document.addEventListener('click', (event) => {
              // Check if the clicked element matches the selector
              if (event.target.matches('#clear-discount-btn')) {
                event.preventDefault();
...
//     if(el.value > maxQuantity) el.value = maxQuantity;
// }

// function validateQtyPlus(){
//     if($('input[name="quantity"]').val() > maxQuantity)
//     {
//         $('input[name="quantity"]').val(maxQuantity);
//     }
// }

// document.getElementById('plus-qty').addEventListener('click', function(e){
//    validateQtyPlus();
// });
/* Cart Coupon Code */
